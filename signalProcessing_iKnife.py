...
for i in range(int(0.05*len(Y_train))):
	...
	rand_int1 = random.randint(0,int(0.25*len(X_train))-i-1)
	...
	rand_int2 = random.randint(int(0.25*len(X_train))-i, int(0.5*len(X_train))-i-1)
	...
	rand_int3 = random.randint(int(0.5*len(X_train))-i, int(0.75*len(X_train))-i-1)
	...
	rand_int4 = random.randint(int(0.75*len(X_train))-i, len(X_train)-i-1)
...
for i, clf in enumerate((svc, lin_svc, rbf_svc, poly_svc)):
	 ...
	 # Plot also the training points
	 plt.scatter(X_test_feat_density, X_test_fft, c=Y_test, cmap=plt.cm.coolwarm)
	 plt.xlabel('X_test_feat_density')
	 plt.ylabel('X_test_fft')
	 plt.xlim(X_train.min(), X_train.max())
	 plt.ylim(yy.min(), yy.max())
	 plt.xticks(())
	 plt.yticks(())
...
Y_pred = svc.predict(X_test_feat_density_and_X_test_fft)
